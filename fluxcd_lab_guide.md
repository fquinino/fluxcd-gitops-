...
### 5.9 Phase 2B: Deploy via Kommander AppDeployment (Workspace-native)

This phase demonstrates NKP-native app propagation using:
* `AppDeployment` (workspace desired state)
* `AppDeploymentInstance` (per-cluster realization)

Use this when you want catalog/governed lifecycle behavior from Kommander.

#### Included manifest

`gitops-config-repo/federation-phase2b-appdeployment/demo-appdeployment.yaml`

Important: replace placeholders before apply:
* `appRef.appID: <your-cluster-app-id>`
* `appRef.name: <your-cluster-app-name-and-version>`

#### Apply phase 2B

```bash
kubectl apply -k ./gitops-config-repo/federation-phase2b-appdeployment
```

#### Validate downstream realization

```bash
# Workspace desired app
kubectl get appdeployment -n nkp-workloads01-cqrp5-5gxht

# Per-cluster instance generated by Kommander
kubectl get appdeploymentinstances -n nkp-workloads01-cqrp5-5gxht
kubectl get appdeploymentinstances -n nkp-workloads01-cqrp5-5gxht -o yaml
```

How to read status:
* `AppDeployment`: cluster selection + desired package/version.
* `AppDeploymentInstance`: resolved per target cluster with:
  * `spec.kommanderClusterRef.name` (target cluster),
  * `status.conditions` including `GitReconciled`, `KustomizationReady`, `KustomizationHealthy`.

Summary:
* **Phase 2 (Kubefed-native)** = best for custom raw Kubernetes resources.
* **Phase 2B (AppDeployment)** = best for NKP catalog-governed downstream installs.
